...
export type MotorEffects = { playerId: string; worldEventId: string; rulesVersion: string; playerExperience: number; playerExperienceDelta: number; currencyDelta: number; playerLevel: number; activities: MotorActivityEffect[]; contractEvidence: Array<{ contractId: string; rationale: string }>; bossEvidence: Array<{ bossId: string; rationale: string }>; entry: StoredDiaryEntry };
