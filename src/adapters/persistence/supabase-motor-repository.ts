...
export class SupabaseMotorRepository implements MotorRepository {
  async resolveGuildCodes(category: string): Promise<string[]> {
    const { data, error } = await createServiceSupabaseClient().from("guild_categories").select("guild_code, guilds!inner(is_active)").eq("category", category).eq("guilds.is_active", true);
