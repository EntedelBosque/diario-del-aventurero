...
export async function loadOracleContextFromSupabase(playerId: string): Promise<OracleContext> {
  ...
  const [stats, entities, contracts, bosses] = await Promise.all([
    ...
    supabase.from("world_entities").select("id, entity_type, canonical_name, aliases, observations").eq("player_id", playerId).is("merged_into_entity_id", null).order("importance_level", { ascending: false }).limit(50),
  ...
  return { language: "es-MX", activeStats, relevantEntities: (entities.data ?? []).map((row) => { const aliases = (row.aliases as string[]) ?? []; return { id: row.id as string, type: row.entity_type as string, name: row.canonical_name as string, aliases, title: aliases[aliases.length - 1], description: (row.observations as string | null) ?? undefined }; }), activeContractIds: (contracts.data ?? []).map((row) => row.id as string), activeBossIds: (bosses.data ?? []).map((row) => row.id as string) };
}
