...
type DiaryRow = { id: string; player_id: string; idempotency_key: string; body: string; occurred_at: string; submitted_at: string; oracle_status: StoredDiaryEntry["oracleStatus"] };
...
export class SupabaseDiaryEntryRepository implements DiaryEntryRepository {
  ...
  private async saveInterpretation(entryId: string, status: "accepted" | "rejected" | "failed", response: OracleResponse | null, errors: string[] | null): Promise<StoredDiaryEntry> {
    ...
    const { data, error } = await this.supabase.from("diary_entries").update({ oracle_status: status }).eq("id", entryId).select().single();
