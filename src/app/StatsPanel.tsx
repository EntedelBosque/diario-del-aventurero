...
export function StatsPanel() {
  ...
  return <section className="stats-panel parchment">
    ...
    <div className="stats-grid">
      {data.stats.map((stat) => <div key={stat.key} className="stat-tile">
    ...
    {data.guilds.length > 0 && <div className="stats-guilds">
      {data.guilds.map((guild) => <span key={guild.code} className="guild-chip">{guildName(guild.code)}<em>{guild.mastery.toLocaleString("es-MX")}</em></span>)}
    </div>}
  </section>;
}
